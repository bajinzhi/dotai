...
export class CodebuddyAdapter extends AbstractToolAdapter {
  constructor(repoLocalPath: string) {
    super(repoLocalPath, {
      toolId: "codebuddy",
      displayName: "CodeBuddy",
      supportedScopes: ["user", "project"],
      repoDirName: "codebuddy",
      scopeConfigs: {
        user: {
          repoSubPath: "",
          targetResolver: (ctx) => path.join(ctx.userHome, ".codebuddy"),
          filePattern:
  ...
  async detect(): Promise<ToolDetectResult> {
    const toolDir = path.join(os.homedir(), ".codebuddy");
    const exists = await fs.pathExists(toolDir);
    return {
      installed: exists,
      location: exists ? toolDir : undefined,
      reason: exists ? undefined : "~/.codebuddy/ directory not found",
    };
  }
  ...
  private resolveTargetPath(
    ...
    if (scope === "user") {
      if (relPath.startsWith("rules/")) {
        return path.join(context.userHome, ".codebuddy", "rules", relPath.slice("rules/".length));
      }
      ...
      return path.join(context.userHome, ".codebuddy", relPath);
    }

    if (relPath.startsWith("rules/")) {
      return path.join(context.projectPath, ".codebuddy", "rules", relPath.slice("rules/".length));
    }
    if (relPath.startsWith("commands/")) {
      return path.join(
        context.projectPath,
    ...
    return path.join(context.projectPath, ".codebuddy", relPath);
  }
}
