...
export class IflowAdapter extends AbstractToolAdapter {
  constructor(repoLocalPath: string) {
    super(repoLocalPath, {
      ...
      scopeConfigs: {
        user: {
          repoSubPath: "",
          targetResolver: (ctx) => path.join(ctx.userHome, ".iflow"),
          filePattern: "{IFLOW.md,rules/**/*.md,commands/**/*,skills/**/*,agents/**/*}",
        },
        project: {
          repoSubPath: "",
  ...
  async getPathMappings(
    ...
    const relPaths = await fg(
      scope === "user"
        ? ["IFLOW.md", "rules/**/*.md", "commands/**/*", "skills/**/*", "agents/**/*"]
  ...
  async detect(): Promise<ToolDetectResult> {
    const toolDir = path.join(os.homedir(), ".iflow");
    const exists = await fs.pathExists(toolDir);
    return {
      installed: exists,
      location: exists ? toolDir : undefined,
      reason: exists ? undefined : "~/.iflow/ directory not found",
    };
  }
  ...
  private resolveTargetPath(
    ...
    if (scope === "user") {
      if (relPath === "IFLOW.md") {
        return path.join(context.userHome, ".iflow", "IFLOW.md");
      }
      if (relPath.startsWith("rules/")) {
        return path.join(context.userHome, ".iflow", "rules", relPath.slice("rules/".length));
      }
      ...
      return path.join(context.userHome, ".iflow", relPath);
    }
    ...
    return path.join(context.projectPath, ".iflow", relPath);
  }
}
